...
localparam S0_IDLE     = 6'b000001;
localparam S0_WAIT2    = 6'b000010;
localparam S0_WAIT1    = 6'b000100;
localparam S0_WAIT_ADD = 6'b001000;
localparam S0_WAIT_MUL = 6'b010000;
localparam S0_WAIT_DIV = 6'b100000;

reg [5:0] s0_current_state, s0_next_state;
...
always @(*) begin
	case(s0_current_state)
		S0_IDLE : begin
			...
			else if(i_valid && isTrans0_add && !haveTrans0_mul && !haveTrans0_div) s0_next_state = S0_WAIT_ADD;
			else if(i_valid && isTrans0_mul && !haveTrans0_add && !haveTrans0_div) s0_next_state = S0_WAIT_MUL;
			else if(i_valid && isTrans0_div && !haveTrans0_add && !haveTrans0_mul) s0_next_state = S0_WAIT_DIV;
		...
		S0_WAIT2  : begin//don't need to wait trans1 or trans0
            ...
            else if(o_ready && isTrans0_div &&  i_valid) s0_next_state = S0_WAIT_DIV;
		...
		S0_WAIT1 : begin
            ...
            else if(s1_ready && isTrans0_div &&  i_valid) s0_next_state = S0_WAIT_DIV;
		...
		S0_WAIT_ADD : begin
            ...
            else if(add_iready && isTrans0_div &&  i_valid) s0_next_state = S0_IDLE    ;
		...
		S0_WAIT_MUL : begin
            ...
            else if(mul_iready && isTrans0_div &&  i_valid) s0_next_state = S0_IDLE    ;
			else if(mul_iready                 && !i_valid) s0_next_state = S0_IDLE    ;
			else	                                        s0_next_state = S0_WAIT_MUL;
		end
		S0_WAIT_DIV : begin
            if     (!s1_ready                             ) s0_next_state = S0_WAIT_DIV;
            else if(div_iready && isTrans2     &&  i_valid) s0_next_state = S0_IDLE    ;
            else if(div_iready && isTrans1     &&  i_valid) s0_next_state = S0_WAIT1   ;
            else if(div_iready && isTrans0_add &&  i_valid) s0_next_state = S0_IDLE    ;
            else if(div_iready && isTrans0_mul &&  i_valid) s0_next_state = S0_IDLE    ;
            else if(div_iready && isTrans0_div &&  i_valid) s0_next_state = S0_WAIT_DIV;
			else if(div_iready                 && !i_valid) s0_next_state = S0_IDLE    ;
			else	                                        s0_next_state = S0_WAIT_DIV;
		end
...
always @(posedge clk or posedge rst) begin
	if(rst) begin
        ...
        if((s0_current_state == S0_WAIT1) && s1_ready) begin
            ...
            else if(div_ovalid && o_ready && haveTrans0_div) cntTrans0_div <= cntTrans0_div - 1;
        ...
        end else if((s0_current_state == S0_WAIT_DIV) && s1_ready && div_iready) begin
            
            if(s1_valid && o_ready && !haveTrans0) cntTrans1 <= cntTrans1 - 1;
            if(div_ovalid && o_ready) cntTrans0_div <= cntTrans0_div    ;
            else                      cntTrans0_div <= cntTrans0_div + 1;
...
assign s0_valid = ((s0_current_state == S0_WAIT_DIV) && s1_ready) || ((s0_current_state == S0_WAIT_MUL) && s1_ready) || ((s0_current_state == S0_WAIT_ADD) && s1_ready) ||
                   (s0_current_state == S0_WAIT2) || (s0_current_state == S0_WAIT1); 
assign s0_ready = ( (s0_current_state == S0_IDLE    ) && ((!haveTrans0 && !haveTrans1) || (isTrans0_add && !haveTrans0_mul && !haveTrans0_div)   ||
                    (isTrans0_mul && !haveTrans0_add && !haveTrans0_div) || (isTrans0_div && !haveTrans0_add && !haveTrans0_mul)              )) ||
                  ( (s0_current_state == S0_WAIT2   ) && o_ready                                                                               ) || 
                  ( (s0_current_state == S0_WAIT1   ) && s1_ready   && !isTrans2                                                               ) ||
                  ( (s0_current_state == S0_WAIT_ADD) && s1_ready   && add_iready && !isTrans2 && !isTrans0_mul && !isTrans0_div               ) ||
                  ( (s0_current_state == S0_WAIT_MUL) && s1_ready   && mul_iready && !isTrans2 && !isTrans0_add && !isTrans0_div               ) ||
                  ( (s0_current_state == S0_WAIT_DIV) && s1_ready   && div_iready && !isTrans2 && !isTrans0_add && !isTrans0_mul               ); 
...
//fdiv
wire div_ivalid = s0_valid && fdiv_d;
wire div_iready = div_iready_a && div_iready_b;
wire div_iready_a;
wire div_iready_b;
...
assign o_valid = (s0_current_state == S0_WAIT2) || s1_valid || s4_valid || div_ovalid;
