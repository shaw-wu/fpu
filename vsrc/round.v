...
wire isUnormalize = (o_exp <= 9'b0_1000_0001);
wire [EXP_BITS-1:0] Unormalize_n = 9'b0_1000_0001 - exp; //-127 - e

wire [SIG_BITS-1:0] shift_sig = i_isUnormalize ? sig >> (Unormalize_n + 1) : sig; //sig[SIG_BITS-1:SIG_BITS-2] = 2e-126, 2e-127
wire [SIG_BITS-1:0] mask = ~({SIG_BITS{1'b1}} << (Unormalize_n + 1));
wire shift_sticky = |(sig & mask) && i_isUnormalize;
...
assign o_exp = of                                    ? of_exp      : 
               i_underUnormal && roundIncre          ? EXPUNOR_MIN : 
/*verilator lint_off WIDTHEXPAND*/
               i_isUnormalize && sig_res[FRA_BITS-1] ? exp + 1     : exp + sig_cout;
/*verilator lint_on WIDTHEXPAND*/
