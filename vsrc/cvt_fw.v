...
module cvt_fw #(
	...
	output [DATA_WIDTH-1:0] w_res	,
	output [DATA_WIDTH-1:0] wu_res  ,
	input  [           2:0] frm	    ,
	input		            isnan   ,
	input		            isinf   ,
	output		            nv	    ,
...
localparam RE_DATAWIDTH = DATA_WIDTH + (1 << (EXP_BITS-1));
...
//overflow
wire of = (u_i && overexp_wu) || (!u_i && overexp_w);
...
//result
wire [DATA_WIDTH-1:0] comp_res = sign ? {1'b1, ~sigres[DATA_BITS-2:0] + 1} : {1'b0, sigres[DATA_BITS-2:0]} ;
wire [DATA_WIDTH-1:0] of_wres  = sign ? {1'b1, {DATA_BITS-1{1'b0}}}        : {1'b0, {DATA_BITS-1{1'b1}}}   ;

assign wu_res = of ? {DATA_WIDTH{1'b1}} : sigres  ;
