...
def is_nan_hex(hex_str):
    """判断一个64位16进制字符串是否为单精度浮点数的 NaN"""
    try:
        val = int(hex_str, 16)
        exponent = (val >> 52) & 0x7FF
        fraction = val & 0x3FFFFFFFFFFFFF
...
def filter_fpu_log(input_file, output_file):
    ...
    pattern = re.compile(r"\[ERR\] id=(\d+) sel=([0-9a-fA-F]+) got=([0-9a-fA-F]+)\s+([0-9a-fA-F]+)\s+exp=([0-9a-fA-F]+)\s+([0-9a-fA-F]+)")
    ...
    with open(input_file, 'r') as f:
        for line in f:
            ...
            if match:
                idx, sel, got_val, got_flags, exp_val, exp_flags = match.groups()
                
                # 条件判断：
                # 1. 你的输出是 Canonical NaN (7fc00000)
                # 2. 预期输出也是某种 NaN
                # 3. fflags 相同
                if (got_val.lower() == "7ff8000000000000" and is_nan_hex(exp_val) and got_flags == exp_flags) or (got_val.lower() == exp_val and got_flags == "00001" and exp_flags == "00000"):
                    removed_count += 1
                    continue # 剔除，不加入结果列表
